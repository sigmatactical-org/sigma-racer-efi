//! STM32F7 brown-out reset option-byte configuration.
//!
//! Register map and `OPTCR` layout: ST RM0410 (STM32F76xxx/F77xxx) §2.2.2 / §3.9.8.
//! FLASH controller @ `0x40023C00`: Zephyr `dts/arm/st/f7/stm32f7.dtsi`
//! (`flash-controller@40023c00`); same base as `stm32_metapac::FLASH`.
//!
//! Option-byte programming mirrors Zephyr `drivers/flash/flash_stm32f7x.c`:
//! `flash_stm32_option_bytes_write()` — masked `OPTCR` write, then `OPTSTRT`.

use defmt::{info, warn};
use crate::safety::BorLevel;
