//! Trigger replay plan — the second MRE as a crank/cam signal generator.
//!
//! Phase 3 of the mule runbook needs synthesized crank + cam waveforms fed
//! into the ECU under test. This module generates the edge schedule as pure
//! `no_std` logic; `src/bin/replay.rs` drives it onto the MRE's ignition
//! outputs (TC4427-buffered 5 V push-pull — ideal drive for the DUT's
//! trigger inputs).
//!
//! The host test closes the loop: a generated plan fed into
//! [`crate::decoder::Decoder`] must reach full sync and track RPM — the
//! signal generator and the decoder validate each other before either
//! touches hardware.

use crate::timing::TriggerWheel;
...
mod tests {
    use super::*;
    use crate::decoder::{Decoder, SyncState};
