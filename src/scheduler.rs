...
//! **timestamps**. This module converts between them: on every tooth the
//! decoder reports position and speed, and the scheduler *predicts* the
//! absolute time of each angle-event falling inside the window up to the
//! next expected tooth, emitting arm requests for hardware compare
//! channels. Acceleration is handled by construction — every tooth
//! re-predicts from the freshest velocity, so errors never accumulate
//! beyond one tooth's horizon.
//!
...
//! no double-fires at window seams, no missed events across the gap (the
//! caller passes the gap's wider window).
//!
//! Pure `no_std`, fixed-capacity, no allocation. The RT layer owns calling
//! this from the crank ISR and loading timer compare registers with the
//! results; sync gating (no spark below `SYNC_FULL`) is the caller's rule,
//! enforced before events are ever loaded here.

/// What a scheduled event drives. Cylinder indices are 0-based.
...
/// Max simultaneous angle-events (3 cylinders × 4 edges + margin).
...
/// The scheduler: a fixed table of angle events plus the prediction math.
#[derive(Debug)]
